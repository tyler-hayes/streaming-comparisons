...
class Reservoir(object):
    # The reservoir sampling algorithm for streaming learning.
    ...
    def fit(self, X_train, y_train, i):
        ...
        if self.counter[y_train] == self.capacity:
            M = np.random.randint(i)
            if M < self.capacity * self.num_classes:
                # find all indices for given class
                idxs = np.where(self.y == y_train)[0]
                # randomly choose one of the indices to replace
