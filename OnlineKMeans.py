...
class OnlineKMeans(object):
    # The online k-means model for streaming learning.
    ...
    def __init__(self, input_shape, num_classes, capacity, cuda_device=None):
        ...
        self.n = np.zeros(capacity * num_classes)
        self.counter = np.zeros((num_classes), dtype=np.int32)
        self.idx_count = 0

    def compute_distance(self, mat, pt):
        """
        Return the index in mat of the closest point based on L2 norm to pt.
        :param mat: matrix of points
        :param pt: point we want distance of
        :return: index of closest point in mat
        """
        pt = np.reshape(pt, (1, self.input_shape))
        d = mat - pt
    ...
    def fit(self, X_train, y_train):
        ...
        if self.counter[y_train] == self.capacity:
            idxs = np.where(self.y == y_train)[0]
            X_tmp = self.X[idxs, :]
            idx = self.compute_distance(X_tmp, X_train)
            idx = idxs[idx]
            self.X[idx, :] = (self.n[idx] * self.X[idx, :] + X_train) / (self.n[idx] + 1)
            self.y[idx] = y_train
            self.n[idx] += 1
        else:
